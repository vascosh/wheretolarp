...
import CitySelectorClient from './CitySelectorClient';
...
export default async function CitySelector() {
  ...
  const slugOrder = ['new-york', 'london', 'miami'];

  const cities = slugOrder.map((slug) => {
  ...
  return <CitySelectorClient cities={cities} />;
}
