...
export default function FeedDMLauncher() {
  ...
  return (
    <button
      ...
      className="group fixed z-30 left-3 sm:left-5 pl-3 pr-4 sm:pl-3.5 sm:pr-5 h-11 sm:h-12 rounded-full border border-forest/25 bg-parchment-light/90 backdrop-blur-md text-forest shadow-[0_4px_24px_rgba(16, 17, 20,0.18)] hover:border-forest/60 hover:bg-parchment-light hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200 flex items-center gap-2"
      style={{ bottom: 'calc(1rem + env(safe-area-inset-bottom))' }}
    >
      <span className="relative inline-flex items-center justify-center w-6 h-6 rounded-full bg-forest/10 text-forest">
        ...
        {unread > 0 && (
          <span
            className="absolute -top-1 -right-1 min-w-[16px] h-4 bg-burgundy rounded-full text-[9px] text-parchment-light flex items-center justify-center font-bold leading-none px-1 ring-2 ring-parchment-light"
            aria-hidden
          >
            {unread > 9 ? '9+' : unread}
          </span>
