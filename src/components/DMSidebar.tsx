...
function PlanCard({ plan, isMe }: { plan: LarpPlan; isMe: boolean }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden min-w-[220px] max-w-[260px] border border-gold/30 bg-parchment-light shadow-[0_2px_16px_rgba(16, 17, 20,0.08)]">
      <div className="px-4 py-3 border-b border-gold/20">
        <div className="flex items-center gap-2 mb-1">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" className="text-gold-dark shrink-0">
            <rect x="3" y="4" width="18" height="18" rx="2" stroke="currentColor" strokeWidth="1.5"/>
            <path d="M16 2v4M8 2v4M3 10h18" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
          </svg>
          <span className="font-sans text-[10px] text-gold-dark tracking-widest uppercase">
            {isInvite ? 'Plan Invite' : 'LARP Plan'}
          </span>
        </div>
        <p className="font-serif text-forest text-sm font-semibold leading-snug">{plan.spot_name}</p>
        {plan.spot_neighborhood && (
          <p className="font-sans text-[11px] text-peat/50 mt-0.5">{plan.spot_neighborhood}</p>
        )}
      </div>
      <div className="px-4 py-2.5">
        {dateStr && (
          <p className="font-sans text-xs text-peat/70 mb-0.5">
            {dateStr}{plan.plan_time ? ` · ${plan.plan_time}` : ''}
          </p>
        )}
        {plan.spot_category && (
          <p className="font-sans text-[10px] text-peat/40 capitalize">{plan.spot_category}</p>
        )}
        {!isMe && (
          <>
            {status === 'accepted' && (
              <p className="font-sans text-[11px] text-forest mt-3">Added to your calendar</p>
            )}
            {status === 'added' && (
              <p className="font-sans text-[11px] text-forest mt-3">Added to your calendar</p>
            )}
            {status === 'declined' && (
              <p className="font-sans text-[11px] text-peat/40 mt-3 italic">Invite declined</p>
            )}
              ...
              isInvite ? (
                <div className="flex gap-2 mt-3">
                  <button
                    ...
                    className="flex-1 py-1.5 rounded-full bg-forest text-parchment-light font-sans text-[11px] tracking-wider uppercase font-semibold hover:bg-forest-light disabled:opacity-50 transition-all">
                    {status === 'loading' ? '…' : 'Accept'}
                  </button>
                  <button
                    onClick={decline}
                    disabled={status === 'loading'}
                    className="flex-1 py-1.5 rounded-full border border-peat/20 text-peat/50 font-sans text-[11px] tracking-wider uppercase hover:text-peat/80 hover:border-peat/40 disabled:opacity-50 transition-all">
                    Decline
                  </button>
                </div>
              ) : (
                <button
                  onClick={accept}
                  disabled={status === 'loading'}
                  className="mt-3 w-full py-1.5 rounded-full bg-forest text-parchment-light font-sans text-[11px] tracking-wider uppercase font-semibold hover:bg-forest-light disabled:opacity-50 transition-all">
        ...
        {isMe && (
          <p className="font-sans text-[10px] text-peat/35 mt-2 italic">Shared with friend</p>
        )}
      </div>
    </div>
  );
...
function MessageBubble({ msg, isMe }: { msg: Message; isMe: boolean }) {
  ...
  return (
    <div className={clsx('max-w-[78%] rounded-3xl px-4 py-2.5 break-words',
      isMe
        ? 'rounded-br-md bg-forest text-parchment-light'
        : 'rounded-bl-md bg-parchment-dark border border-peat/10 text-peat')}>
      ...
      {msg.message_type === 'file' && msg.media_url && (
        <a href={msg.media_url} target="_blank" rel="noopener noreferrer"
          ...
          <div className={clsx('w-8 h-8 rounded-lg flex items-center justify-center shrink-0',
            isMe ? 'bg-parchment-light/15' : 'bg-gold/15')}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none"
              className={isMe ? 'text-gold-light' : 'text-gold-dark'}>
              <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" stroke="currentColor" strokeWidth="1.5"/>
              <path d="M14 2v6h6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
            </svg>
          </div>
          <span className={clsx('font-sans text-xs transition-colors truncate max-w-[160px]',
            isMe ? 'text-gold-light hover:text-parchment-light' : 'text-gold-dark hover:text-forest')}>
            {msg.media_name ?? 'File'}
          </span>
        </a>
      )}
      ...
      {msg.message_type === 'link' && msg.content && (
        <a href={msg.content} target="_blank" rel="noopener noreferrer"
          className={clsx('font-sans text-sm underline underline-offset-2 break-all transition-colors',
            isMe ? 'text-gold-light hover:text-parchment-light' : 'text-gold-dark hover:text-forest')}>
          {msg.content}
        </a>
      )}
...
export default function DMSidebar({ isOpen, onClose, initialConvId }: DMSidebarProps) {
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className="fixed inset-0 z-[400] bg-peat/25 backdrop-blur-[2px]"
        onClick={() => { if (view === 'thread') backToList(); else onClose(); }}
      />

      {/* Floating paper panel — softly rounded, detached from screen edges.
          On phones, sit close to the bottom (the home indicator) so the panel
          fills the visible area; on tablet/desktop float between the nav and bottom. */}
      <div
        className="fixed z-[401] flex flex-col left-2 right-2 sm:left-auto sm:right-6 sm:w-[400px] rounded-3xl overflow-hidden border border-peat/15 bg-parchment-light shadow-[0_20px_60px_rgba(16, 17, 20,0.28)]"
        style={{
      ...
      >
        ...
        {view === 'list' && (
          <>
            <div className="px-5 py-5 border-b border-forest/15 flex items-center justify-between shrink-0">
              <div>
                <p className="eyebrow mb-1">Private Wire</p>
                <h2 className="headline-editorial text-2xl">Messages</h2>
                {totalUnread > 0 && (
                  <p className="font-sans text-[10px] text-burgundy mt-1 tracking-[0.2em] uppercase">{totalUnread} unread</p>
                )}
              </div>
              <button onClick={onClose}
                className="text-peat/35 hover:text-forest transition-colors p-2 rounded-full hover:bg-peat/[0.06]">
            ...
            <div className="flex-1 overflow-y-auto">
              {loadingConvs ? (
                ...
                <div className="flex flex-col items-center justify-center h-60 px-8 text-center gap-4">
                  <div className="w-14 h-14 rounded-full flex items-center justify-center border border-gold/25 bg-gold/[0.08]">
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" className="text-gold-dark/70">
                      <path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                  </div>
                  <div>
                    <p className="eyebrow mb-2">The wire is silent</p>
                    <p className="font-sans text-peat/50 text-xs leading-relaxed">
                ...
                <div className="divide-y divide-forest/10">
                  {conversations.map(conv => (
                    <button key={conv.id} onClick={() => openConversation(conv)}
                      className="w-full px-4 py-4 flex items-center gap-3 hover:bg-parchment-dark/40 transition-all text-left">
                      <div className="relative shrink-0">
                        <Avatar name={conv.other.name} image={conv.other.avatar_url} size={44} />
                        {conv.unreadCount > 0 && (
                          <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] bg-burgundy rounded-full text-[9px] text-parchment-light flex items-center justify-center font-bold px-0.5">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-0.5">
                          <p className={clsx('font-serif text-sm truncate',
                            conv.unreadCount > 0 ? 'text-forest font-semibold' : 'text-peat/75')}>
                            {conv.other.name ?? conv.other.username ?? 'Unknown'}
                          </p>
                          {conv.lastMessage && (
                            <span className="font-sans text-[10px] text-peat/40 shrink-0 ml-2">
                        ...
                        {conv.lastMessage && (
                          <p className={clsx('font-sans text-xs truncate',
                            conv.unreadCount > 0 ? 'text-peat/70' : 'text-peat/45')}>
                            {conv.lastMessage.isFromMe && <span className="text-peat/35">You: </span>}
        ...
        {view === 'thread' && selectedConv && (
          <>
            {/* Thread header */}
            <div className="px-4 py-4 border-b border-forest/15 flex items-center gap-3 shrink-0">
              <button onClick={backToList}
                className="text-peat/40 hover:text-forest transition-colors p-1.5 rounded-full hover:bg-peat/[0.06] shrink-0">
              ...
              <Link href={`/u/${selectedConv.other.id}`} onClick={onClose} className="flex-1 min-w-0 hover:opacity-80 transition-opacity">
                <p className="font-serif text-sm text-forest truncate">
                  {selectedConv.other.name ?? selectedConv.other.username ?? 'Unknown'}
                </p>
                {selectedConv.other.username && (
                  <p className="font-sans text-[10px] tracking-[0.15em] uppercase text-gold-dark/70 leading-none mt-0.5">@{selectedConv.other.username}</p>
                )}
              </Link>
              <button onClick={onClose}
                className="text-peat/30 hover:text-peat/60 transition-colors p-1.5 rounded-full shrink-0">
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
            ...
            <div className="flex-1 overflow-y-auto px-4 py-5 space-y-1.5">
              {loadingMsgs ? (
                ...
                <div className="flex flex-col items-center justify-center h-48 text-center gap-2">
                  <p className="font-display italic text-peat/40 text-sm">Open the correspondence.</p>
                </div>
              ) : (
                <>
                  {messages.map((msg, i) => {
                      ...
                      <div key={msg.id} className={clsx(!sameAsPrev && i > 0 ? 'mt-3' : '')}>
                        {showTimestamp && (
                          <div className="text-center py-2">
                            <span className="font-sans text-[10px] text-peat/40 px-3 py-1 rounded-full bg-peat/[0.05]">
            ...
            {/* Plan picker modal */}
            {showPlanPicker && (
              <div className="mx-4 mb-2 rounded-xl overflow-hidden border border-gold/30 bg-parchment">
                <div className="px-4 py-3 border-b border-gold/20 flex items-center justify-between">
                  <p className="font-serif text-forest text-xs font-semibold">Share a Plan</p>
                  <button onClick={() => setShowPlanPicker(false)}
                    className="text-peat/40 hover:text-peat/70 transition-colors">
                ...
                <div className="max-h-48 overflow-y-auto">
                  {loadingPlans ? (
                    ...
                    <p className="font-sans text-peat/40 text-xs text-center py-6">No plans yet</p>
                  ) : (
                    myPlans.map(plan => (
                      <button key={plan.id} onClick={() => selectPlan(plan)}
                        className="w-full px-4 py-3 text-left hover:bg-gold/[0.07] transition-all border-b border-forest/10 last:border-0">
                        <p className="font-serif text-peat text-xs font-semibold truncate">{plan.spot_name}</p>
                        <p className="font-sans text-[10px] text-peat/45 mt-0.5">
                          {new Date(plan.plan_date + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
            ...
            {pendingPlan && (
              <div className="mx-4 mb-2 px-3 py-2.5 rounded-xl flex items-center gap-3 border border-gold/30 bg-gold/[0.08]">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" className="text-gold-dark shrink-0">
                  <rect x="3" y="4" width="18" height="18" rx="2" stroke="currentColor" strokeWidth="1.5"/>
                  <path d="M16 2v4M8 2v4M3 10h18" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                </svg>
                <div className="flex-1 min-w-0">
                  <p className="font-sans text-xs text-peat font-semibold truncate">{pendingPlan.spot_name}</p>
                  <p className="font-sans text-[10px] text-peat/45">
                ...
                <button onClick={() => setPendingPlan(null)}
                  className="text-peat/40 hover:text-peat/70 transition-colors p-1 shrink-0">
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                    <path d="M3 3L11 11M11 3L3 11" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                  </svg>
                </button>
            ...
            {/* Attachment preview */}
            {attached && (
              <div className="mx-4 mb-2 px-3 py-2 rounded-xl flex items-center gap-3 border border-peat/10 bg-peat/[0.04]">
                {attached.type === 'image' && attached.preview ? (
                  ...
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0 bg-gold/[0.12]">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" className="text-gold-dark/70">
                      <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" stroke="currentColor" strokeWidth="1.5"/>
                      <path d="M14 2v6h6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                    </svg>
                  </div>
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-sans text-xs text-peat/70 truncate">{attached.name}</p>
                  <p className="font-sans text-[10px] text-peat/40">
            ...
            {/* Composer */}
            <div className="px-4 pt-2 pb-3 border-t border-forest/15 shrink-0">
              <div className="flex items-end gap-2">
                <button onClick={() => fileInputRef.current?.click()}
                  className="p-2.5 rounded-full text-peat/40 hover:text-gold-dark hover:bg-gold/[0.08] transition-all shrink-0 mb-0.5"
                  title="Attach file">
                ...
                <button onClick={openPlanPicker}
                  className={clsx(
                    ...
                    showPlanPicker || pendingPlan
                      ? 'text-gold-dark bg-gold/[0.14]'
                      : 'text-peat/40 hover:text-gold-dark hover:bg-gold/[0.08]'
                  )}
                  title="Share a plan">
                ...
                <textarea
                  ...
                  placeholder="Message…"
                  rows={1}
                  className="flex-1 resize-none rounded-2xl px-3.5 py-2.5 font-sans text-sm text-peat placeholder:text-peat/35 bg-peat/[0.05] border border-peat/15 focus:outline-none focus:border-gold/50 transition-all overflow-hidden"
                  style={{
                    lineHeight: '1.5',
                    minHeight: '40px',
                    maxHeight: '120px',
                ...
                <button onClick={handleSend}
                  ...
                  className={clsx(
                    ...
                    (text.trim() || attached || pendingPlan) && !sending
                      ? 'bg-forest text-parchment-light hover:bg-forest-light shadow-sm'
                      : 'bg-peat/[0.06] text-peat/30 cursor-not-allowed'
                  )}>
                  {sending ? (
              ...
              <p className="font-sans text-[10px] text-peat/35 mt-2 text-center">
